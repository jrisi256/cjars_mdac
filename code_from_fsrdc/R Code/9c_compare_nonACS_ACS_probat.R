...
################################################################################
table <- "probat"
d_bcol <- "PRO_BGN_DT_DD"
m_bcol <- "PRO_BGN_DT_MM"
y_bcol <- "PRO_BGN_DT_YYYY"
d_ecol <- "PRO_END_DT_DD"
m_ecol <- "PRO_END_DT_MM"
y_ecol <- "PRO_END_DT_YYYY"
nomdac_table <- "nomdac_probat"
mdac_table <- "mdac_probat"
mdac_alt_table <- "mdac_alt_probat"
end_status_col <- "PRO_END_CD"
end_status_plot_title <- "Probation end status"
state_col <- "PRO_ST_ORI_FIPS"
county_col <- "PRO_CNTY_ORI_FIPS"
county_year_yaxis <- "Percent of probations"
year_xaxis <- "Year of probation entry"

################################################################################
# Compare probation between non-MDAC-CJARS and MDAC-CJARS.
################################################################################
incident_level <-
  pmap(
    list(
...
################################################################################
# Compare proportions of individuals who had a probation entry.
################################################################################
...
################################################################################
# Compare average probation length and probation length distribution.
################################################################################
...
################################################################################
# Compare number of probation spells between MDAC and non-MDAC.
################################################################################
...
################################################################################
# Compare probation conditions.
################################################################################
probat_conditions <-
  bind_rows(incident_level) %>%
  lazy_dt() %>%
  count(sample, PRO_COND_CD) %>%
  group_by(sample) %>%
  mutate(prcnt = n / sum(n) * 100) %>%
  ungroup() %>%
  as_tibble()

ggplot(probat_conditions, aes(x = fct_reorder(PRO_COND_CD, prcnt), y = prcnt)) +
  geom_bar(stat = "identity") +
  theme_bw() +
  labs(x = "Probation conditions", y = "Percent") +
  facet_wrap(~sample)

probat_conditions_revise <-
  probat_conditions %>%
  group_by(sample) %>%
  mutate(total_nr_probat = sum(n)) %>%
  ungroup() %>%
  pivot_wider(
    id_cols = c("sample", "total_nr_probat"),
    names_from = PRO_COND_CD,
    values_from = c("n", "prcnt")
  )

probat_chisq_test_cond <-
  chisq.test(
    bind_rows(
      probat_conditions_revise %>% filter(sample == "nomdac_probat") %>% select(matches("n_")),
      probat_conditions_revise %>% filter(sample == "mdac_probat") %>% select(matches("n_"))
    ),
    correct = F
  )

probat_chisq_test_alt_cond <-
  chisq.test(
    bind_rows(
      probat_conditions_revise %>% filter(sample == "nomdac_probat") %>% select(matches("n_")),
      probat_conditions_revise %>% filter(sample == "mdac_alt_probat") %>% select(matches("n_"))
    ),
    correct = F
  )

probat_prop_test_cond <-
  pmap(
    list(
      sample1 = list("nomdac_probat", "nomdac_probat", "nomdac_probat", "nomdac_probat"),
      sample2 = list("mdac_probat", "mdac_probat", "mdac_probat", "mdac_probat"),
      column = list("n_AD", "n_PR", "n_SP", "n_UU")
    ),
    conduct_prop_test,
    df = probat_conditions_revise,
    n_column = "total_nr_probat"
  )
names(probat_prop_test_cond) <- c("n_AD", "n_PR", "n_SP", "n_UU")

probat_prop_test_alt_cond <-
  pmap(
    list(
      sample1 = list("nomdac_probat", "nomdac_probat", "nomdac_probat", "nomdac_probat"),
      sample2 = list("mdac_alt_probat", "mdac_alt_probat", "mdac_alt_probat", "mdac_alt_probat"),
      column = list("n_AD", "n_PR", "n_SP", "n_UU")
    ),
    conduct_prop_test,
    df = probat_conditions_revise,
...
names(probat_prop_test_alt_cond) <- c("n_AD", "n_PR", "n_SP", "n_UU")

################################################################################
# Compare probation end statuses.
################################################################################
end_status <-
  bind_rows(incident_level) %>%
  lazy_dt() %>%
  ...
  mutate(
    end_status =
      case_when(
        ...
        .data[[end_status_col]] == "IN" ~ "incarceration",
        .data[[end_status_col]] %in% c("AB", "DE", "TR", "OT", "OU", "DI") ~ "other"
      )
  ) %>%
