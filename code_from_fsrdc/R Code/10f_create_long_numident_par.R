...
numident <-
  read_csv(
    ...
    col_select =
      matches(
        "pik|year|gender|race|pobst|nr_parole_(clean|dirty)|citizen|nr_flags|contact"
      )
  )
...
numident_clean <-
  numident %>%
  lazy_dt() %>%
  # Drop individuals where all their paroles have date errors.
  filter(nr_parole_clean != 0 | nr_parole_dirty == 0) %>%
...
#states_long <- list(c("TX"), c("FL"), c("MI"), c("NC"))
cjars_long <- as.list(rep("par", length(states_long)))
...
long_data_df <-
  pmap(
    ...
    create_long_data,
    df = select(numident_clean, -matches("nr_parole")),
...
################################################################################
par_file <-
  file.path("par_long_numident.rds")

if(!file.exists(par_file)) {
  par <-
    read_csv(
      file.path(read_dir, "par.csv"),
      col_select = matches("pik|PAR_ID|DT_YYYY|date[12]$|PAR_END_CD")
    )
  ...
  par_long <-
    ...
    mutate(
      begin_date =
        case_when(
          ...
          # parole, use the real begin date.
          year == start_year ~ date1,
          # If the year of the row we are on corresponds to the end of the
          # parole, set the start date as the beginning of the year.
      ...
      end_date =
        case_when(
          ...
          # If the year of the row we are on corresponds to the beginning of the
          # parole, set the end date as the end of the year.
          year == start_year ~ ymd(paste0(year, "-12-31")),
          # If the year of the row we are on corresponds to the end of the
          # parole, set the start date as the real end date.
          year == end_year ~ date2,
          # If the year of the row we are on corresponds to neither the end nor
    ...
    pivot_wider(
      names_from = PAR_END_CD,
      values_from = temp,
      values_fill = 0,
      names_prefix = "par_end_cd_"
    ) %>%
    # Find overlapping parole spells for a given person for a given year.
    group_by(pik, year) %>%
    mutate(group = iv_identify_group(range)) %>%
    group_by(pik, year, group) %>%
    summarise(n = n(), across(matches("par_end_cd"), function(col) {sum(col)})) %>%
    ungroup() %>%
    mutate(
      begin_date = iv_start(group),
      end_date = iv_end(group),
      nr_days_par = as.numeric(end_date - begin_date) + 1,
      # If there is only 1 parole spell for a given range, there is no overlap.
      overlap = if_else(n == 1, 0, n)
    ) %>%
    # For each person for each year, count the number of days on parole, the
    # number of unique parole spells, and the number of overlapping parole spells..
    group_by(pik, year) %>%
    summarise(
      nr_par_spells = sum(n),
      nr_par_days = sum(nr_days_par),
      overlap_par = sum(overlap),
      across(matches("par_end_cd"), function(col) {sum(col)})
    ) %>%
    ungroup()
  
  saveRDS(par_long, par_file)
} else {
  par_long <- readRDS(par_file)
}
