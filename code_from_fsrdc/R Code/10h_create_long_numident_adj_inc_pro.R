...
pro_long <- readRDS(file.path(long_dir, "pro_long_numident.rds"))
...
#states_long <- list(c("TX"), c("MI"), c("WI"), c("NC"))
cjars_long <- rep(list(c("adj", "inc", "pro")), length(states_long))
...
################################################################################
cjars_tables_all <-
  list(adj_long, inc_long, pro_long) %>%
...
punishment <-
  ...
  summarise(
    ...
    nr_gf_adj = sum(felony_guilty_gradeDisp),
    nr_inc = sum(nr_inc_spells_all_inc),
    nr_pro = sum(nr_pro_spells)
  ) %>%
  ungroup() %>%
  as_tibble()
...
guilty_non_felony <-
  punishment %>%
  filter(nr_guilty >= 1 & nr_gf_adj == 0) %>%
  mutate(
    inc_or_pro_punish = nr_inc >= 1 | nr_pro >= 1,
    inc_punish_only = nr_inc >= 1 & nr_pro == 0,
    pro_punish_only = nr_pro >= 1 & nr_inc == 0,
    inc_and_pro_punish = nr_inc >= 1 & nr_pro >= 1
  )
