...
par <- read_csv(file.path(wide_dir, "par_cj-event-level.csv"))

mdac_cjars_wide <-
  read_csv(
    file.path(wide_dir, "mdac_cjars_wide.csv"),
    col_select = matches("pik|nr_par_(dirty|clean)")
  )
...
focal_states_sample_clean <-
  ...
  # Drop individuals who only had paroles with problematic dates.
  filter(nr_par_clean != 0 | nr_par_dirty == 0) %>%
  select(-matches("nr_"))
...
states_long <-
  list(c("TX"), c("FL"), c("MI"), c("NC"), c("TX", "FL", "MI", "NC"))

cjars_long <- list(c("par"), c("par"), c("par"), c("par"), c("par"))
...
par_long <-
  ...
  ungroup()
write_csv(par_long, file.path(input_dir, "par_long_mdac.csv"))
  ...
  map(
    ...
    function(mdac_long, cjars) {
      cjars %>%
        ...
        mutate(
          across(
            matches("nr_par|overlap|par_end_cd"),
...
pwalk(
  list(par_long_final, names(par_long_final)),
