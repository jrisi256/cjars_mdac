...
par_long <- readRDS(file.path(long_dir, "par_long_numident.rds"))
...
numident_clean <-
  ...
  filter(
    ...
    (nr_parole_dirty != nr_parole_total | nr_parole_total == 0)
  ) %>%
...
#states_long <- list(c("TX"), c("MI"), c("NC"))
cjars_long <- rep(list(c("adj", "inc", "pro", "par")), length(states_long))
...
################################################################################
cjars_tables_all <-
  list(adj_long, inc_long, pro_long, par_long) %>%
  ...
  relocate(
    c(
      ...
      "nr_pro_spells", "nr_par_days", "nr_par_spells"
    ),
    .after = pik
  )
