...
par_long <- read_csv(file.path(input_dir, "par_long_mdac.csv"))
...
focal_states_sample_clean <-
  ...
  filter(
    ...
    (nr_par_dirty != nr_par_total | nr_par_total == 0)
  ) %>%
...
cjars_long <-
  list(
    c("adj", "inc", "par"), c("adj", "inc", "par"), c("adj", "inc", "par"),
    c("adj", "inc", "par"), c("adj", "inc", "par")
  )
